...
from schemas import CompanyGoalSkillsConfig
...
def create_company_goal_skills(config: CompanyGoalSkillsConfig) -> Path:
    logger.info(f"Processing company goals from {config.company_goals_path}")

    goals_df = pd.read_csv(config.company_goals_path)
    if goals_df.empty:
    ...
    save_path = save_dataframe_to_csv(company_goal_skills_df, config.output_path)
